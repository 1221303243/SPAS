...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $total_weightage = array_sum($weightages);
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO assessment_plans (subject_id, assessment_type, weightage) VALUES (?, ?, ?)");
        
        for ($i = 0; $i < count($assessment_types); $i++) {
            $stmt->bind_param("isi", $subject_id, $assessment_types[$i], $weightages[$i]);
            $stmt->execute();
            ...
            $event_text = "Assessment Due: " . $assessment_types[$i] . " for " . $subject_name;
